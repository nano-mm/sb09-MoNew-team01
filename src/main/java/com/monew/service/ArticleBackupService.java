...
public class ArticleBackupService {
  ...
  public void export(LocalDateTime from, LocalDateTime to) {
    ...
    try {
      ...
      List<ArticleInterest> targetMappings = articleInterestRepository.findAllByArticleInWithInterest(
          articles);
      ...
      Map<UUID, Map<String, List<String>>> articleToInterestsMap = targetMappings.stream()
          .collect(Collectors.groupingBy(
              ...
              Collectors.toMap(
                  ...
                  ai -> ai.getInterest().getKeywords() != null ? ai.getInterest().getKeywords()
                      : new ArrayList<>(),
      ...
      log.error("[뉴스 기사] 백업 중 치명적인 오류 발생", e);
      throw new RuntimeException(e);
    }
  }

  @Transactional
  public ArticleRestoreResultDto importBackup(LocalDateTime from, LocalDateTime to){
    try {
      ...
      for (Resource resource : backupResources) {
        ...
        try (InputStream is = resource.getInputStream()) {
          ...
          Set<String> existingInDbUrls = backupUrls.isEmpty() ? Collections.emptySet()
              : articleRepository.findExistingUrls(backupUrls);

          for (ArticleBackupDto dto : backupList) {
            Instant articleDate = dto.publishDate();
            if (articleDate == null || articleDate.isBefore(fromInstant) || articleDate.isAfter(
                toInstant))
              continue;
            ...
            Map<String, List<String>> keywordsMap =
                dto.interestKeywords() != null ? dto.interestKeywords() : Collections.emptyMap();

            for (Map.Entry<String, List<String>> entry : keywordsMap.entrySet()) {
              String interestName = entry.getKey();
              List<String> keywords =
                  entry.getValue() != null ? entry.getValue() : new ArrayList<>();
      ...
      log.error("[뉴스 기사] 데이터 복구 중 치명적인 오류 발생", e);
      throw new RuntimeException(e);
    }
  }
}
